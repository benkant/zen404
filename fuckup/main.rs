...
fn main() {
    ...
    // Load schema
    let schema_str = std::fs::read_to_string(&schema_path).unwrap_or_else(|_| {
        eprintln!("Unable to read schema at {:?}", schema_path);
        std::process::exit(2);
    });
    let schema_json: Value = serde_json::from_str(&schema_str).unwrap_or_else(|_| {
        eprintln!("Unable to parse schema JSON at {:?}", schema_path);
        std::process::exit(2);
    });
    let compiled =
        JSONSchema::options().with_draft(Draft::Draft7).compile(&schema_json).unwrap_or_else(|e| {
